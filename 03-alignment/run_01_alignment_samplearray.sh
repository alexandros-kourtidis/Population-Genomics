...
bwa mem -t 10 -M $REF \
        $IN_DIR/$(echo "${samples[ID]}")/$(echo "${samples[ID]}")_1_trimmed.fq.gz  \
        $IN_DIR/$(echo "${samples[ID]}")/$(echo "${samples[ID]}")_2_trimmed.fq.gz  \
| samtools view -bS > $OUT_DIR/$(echo "${samples[ID]}")_aligned.bam
 
# Filter using samtools
samtools view -f 0x02 -q 20 -b $OUT_DIR/$(echo "${samples[ID]}")_aligned.bam > batch4_alignments/$(echo "${samples[ID]}")_aligned.filtered.bam
 
# Sort using samtools
samtools sort $OUT_DIR/$(echo "${samples[ID]}")_aligned.filtered.bam -o batch4_alignments/$(echo "${samples[ID]}")_aligned.sorted.bam
...
samtools flagstat $OUT_DIR/$(echo "${samples[ID]}")_aligned.bam > batch4_alignments/$(echo "${samples[ID]}")_alignment.flagstat.txt
samtools flagstat $OUT_DIR/$(echo "${samples[ID]}")_aligned.sorted.nd.bam > batch4_alignments/$(echo "${samples[ID]}")_alignment.sorted.nd.flagstat.txt
echo "done"
