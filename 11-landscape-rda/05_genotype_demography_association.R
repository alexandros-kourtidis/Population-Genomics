...
RDAdemo <- rda(geno ~ pi + Ne_LDgenome + mit_diversity + 
...
str(indv)
save(RDAdemo, file = "RDAdemo_model.RData")
load("RDAdemo_model.RData")

# -------------------------------------------------------------
# 1. Screeplot & Biplot of SNPs without outliers
# -------------------------------------------------------------

screeplot(RDAdemo, main="Eigenvalues of constrained axes")

# Extract scores
TAB_loci <- as.data.frame(scores(RDAdemo, display = "species", scaling = 2))
TAB_var  <- as.data.frame(scores(RDAdemo, display = "bp",      scaling = 2))
...
#p_biplot
ggsave("biplot_RDAdemo_RDA12.png", p_biplot, height=6, width=10, dpi=100)
...
TAB_loci <- as.data.frame(scores(RDAdemo, display = "species", scaling = 2, choices = 1:3))
TAB_var  <- as.data.frame(scores(RDAdemo, display = "bp",      scaling = 2, choices = 1:3))
...
p
htmlwidgets::saveWidget(p, "biplot_3d_RDAdemo.html", selfcontained = TRUE)
...
# Extract scores
TAB_sites <- as.data.frame(scores(RDAdemo, display = "sites", scaling = 2))
TAB_var   <- as.data.frame(scores(RDAdemo, display = "bp",    scaling = 2))
